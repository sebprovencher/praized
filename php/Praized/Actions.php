<?php
/**
 * Praized Portable PHP Library: Actions
 * 
...
if ( ! class_exists('PraizedActions') ) {
    require_once(dirname(realpath(__FILE__)).'/Core.php');
    
    /**
     * Praized Portable PHP Library: Actions: Class
     * 
     * Note: Using the OAuth functionalities will make this library PHP5+ only
     * 
     * @package Praized
     * @since 0.1
     */
    class PraizedActions extends PraizedCore {
    	/**
    	 * Constructor
    	 *
    	 ...
    	 * @return PraizedActions
    	 * @since 0.1
    	 */
        function PraizedActions($community, $apiKey, &$oAuth, $consumerKey = null, $consumerSecret = null) {
    		if ( ! is_object($oAuth)) {
    	...
    	/**
    	 * Returns a list of actions objects, optionally based on the submitted query
    	 *
    	...
    	function get($query = array(), $rawJson = false) {
    	    if ( $json = $this->_get('/actions.json', $query) )
